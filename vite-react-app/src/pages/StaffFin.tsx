...
export default function StaffFin() {
  ...
  const fetchFinancialAidRequests = async () => {
    try {
      const response = await fetch("http://localhost:8080/api/aid/type/Pending", {
        method: "GET",
      });
  ...
  const handleUpdateType = async (id: string, newType: string) => {
    try {
      console.log(`PUT Request URL: http://localhost:8080/api/aid/${id}/type`);
      console.log("Request Body:", newType);

      const response = await fetch(`http://localhost:8080/api/aid/${id}/type`, {
        method: "PUT",
        headers: {
